...
async function getAccessToken() {
  ...
  if (!res.ok) {
    throw new Error(`YOUTUBE_OAUTH_HTTP_${res.status}`);
  }
...
export async function publishVideoToYouTube({
  ...
  if (!initRes.ok) {
    throw new Error(`YOUTUBE_UPLOAD_INIT_HTTP_${initRes.status}`);
  }
  ...
  if (!uploadRes.ok) {
    throw new Error(`YOUTUBE_UPLOAD_HTTP_${uploadRes.status}`);
  }
